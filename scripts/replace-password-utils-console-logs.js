...
/**
 * Script to replace all console statements in passwordUtils.js
 */

const fs = require('fs');
const path = require('path');

const passwordUtilsFile = path.join(__dirname, '../utils/passwordUtils.js');
let content = fs.readFileSync(passwordUtilsFile, 'utf8');

// Replacements for console statements
const replacements = [
  {
    search: "console.log('Generating password reset token for user:', user._id);",
    replace: "logInfo('Generating password reset token', { \n      userId: user._id \n    });",
  },
  {
  ...
  {
    search: "console.log('Verifying password reset token:', token);",
    replace:
  ...
  {
    search: "console.log('Token found and valid:', resetToken);",
    replace:
      "logInfo('Token found and valid', { \n      tokenId: resetToken._id,\n      userId: resetToken.user \n    });",
  },
  {
    search: "console.log('Generating password reset URL');",
    replace: "logDebug('Generating password reset URL');",
  },
  {
  ...
  {
    search: "console.log('Generated password reset URL:', resetUrl);",
    replace: "logInfo('Generated password reset URL', { \n      resetUrl \n    });",
  },
  {
    search: "console.error('User not found for audit log:', userId);",
    replace: "logError('User not found for audit log', null, { \n      userId \n    });",
  },
  {
    search: "console.log('Password change audit log created for user:', userId);",
    replace: "logInfo('Password change audit log created', { \n      userId \n    });",
  },
  {
    search: "console.error('Error creating password change audit log:', error);",
    replace:
...
fs.writeFileSync(passwordUtilsFile, content, 'utf8');

console.log('✅ Successfully replaced all console statements in passwordUtils.js');
