...
const { authenticateJWT, requireAuth, requireEmailVerified } = require("../../../middleware/jwtAuth");
...
router.get("/profile", authenticateJWT, requireAuth, requireEmailVerified, catchAsync(users.getUser));

// Update user profile
router.put("/profile", authenticateJWT, requireAuth, requireEmailVerified, catchAsync(users.updateProfile));

// Submit a contact form
router.post("/contact", authenticateJWT, requireAuth, requireEmailVerified, catchAsync(users.submitContact));

// Get user reviews
router.get("/reviews", authenticateJWT, requireAuth, requireEmailVerified, catchAsync(users.getUserReviews));
